...
An example of a situation where you would construct a static graph instead of an interactive graph would be maybe when you are trying to show averages or variability of the different majors in the data.
...
ui <- fluidPage(
  sidebarLayout(sidebarPanel(
    ...
    selectizeInput("secondplayer",
                   label = "Choose a 2nd Player", choices = levels(factor(df$player)),
...
server <- function(input, output, session) {
  ...
  df_sub2 <- reactive({
    df1 <- df %>% filter(player == input$playerchoice)
    df2 <- df %>% filter(player == input$secondplayer)
    bind_rows(df1, df2)
  })
